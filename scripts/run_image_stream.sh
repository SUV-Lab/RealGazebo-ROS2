...
# fall back to vehicle_<num> when "realgazebo" is not running.
#
...
elif docker ps --format '{{.Names}}' | grep -qx "vehicle_${VEHICLE_NUM}"; then
    CONTAINER="vehicle_${VEHICLE_NUM}"   # manager multi-container mode
else
    echo "No simulation container found (looked for 'realgazebo' and 'vehicle_${VEHICLE_NUM}')."
    exit 1
fi
